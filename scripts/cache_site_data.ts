...
function parseLapTime(value: string | null | undefined) {
  ...
  if (parts.length === 2) return Number((parts[0] * 60 + parts[1]).toFixed(3));
  if (parts.length === 1) return Number(parts[0].toFixed(3));
  return null;
}

function formatTimeFromSeconds(value: number | null) {
  if (!Number.isFinite(Number(value))) return null;
  const sec = Number(value);
  const min = Math.floor(sec / 60);
  const rest = (sec % 60).toFixed(3).padStart(6, "0");
  return min > 0 ? `${min}:${rest}` : `${rest}`;
}

function buildLaps(lapsRace: any, driverIdToNumber: Map<string, number>) {
  const laps: any[] = [];
  const positions: any[] = [];
...
function buildPitStops(pitRace: any, driverIdToNumber: Map<string, number>) {
  const pitStops: any[] = [];
  for (const stop of pitRace?.PitStops || []) {
...
function buildDriverSummaries(drivers: any[], laps: any[], pitStops: any[], positions: any[]) {
  return drivers.map((driver) => {
    ...
    const bestLapText = driver.best_lap_text || formatTimeFromSeconds(bestLap);
...
function mergeDashboard(existing: any, session: any, race: any, drivers: any[], laps: any[], positions: any[], pitStops: any[]) {
  ...
  return {
    ...
    data_quality: {
      ...
      source: "OpenF1 session index + Jolpica full race API",
      ...
      note: "Для сайта загружены основные данные через API: результат, старт, круги, позиции по кругам, пит-стопы, лучший круг. Карта/радио/интервалы зависят от отдельных источников и могут быть недоступны.",
    },
  };
}
...
async function main() {
  ...
  console.log(`Caching complete site data for ${year} from OpenF1 + Jolpica...`);
  ...
  for (const race of races) {
    ...
    const drivers = (race.Results || []).map(driverFromResult).filter((driver: any) => Number.isFinite(Number(driver.driver_number)));
    const driverIdToNumber = new Map(drivers.map((driver: any) => [driver.driver_id, Number(driver.driver_number)]));

    const [qualifyingRaw, lapsRaw, pitRaw] = await Promise.all([
      optionalJson(`${JOLPICA_BASE}/${year}/${round}/qualifying.json?limit=200`),
      optionalJson(`${JOLPICA_BASE}/${year}/${round}/laps.json?limit=5000`),
      optionalJson(`${JOLPICA_BASE}/${year}/${round}/pitstops.json?limit=500`),
    ]);

    const qualifyingRace = qualifyingRaw?.MRData?.RaceTable?.Races?.[0];
    const grid = qualifyingGrid(qualifyingRace);
    for (const driver of drivers) {
      if (!driver.starting_position && driver.driver_id && grid.has(driver.driver_id)) {
        driver.starting_position = grid.get(driver.driver_id) || null;
      }
    }

    const lapsRace = lapsRaw?.MRData?.RaceTable?.Races?.[0];
    const { laps, positions } = buildLaps(lapsRace, driverIdToNumber);
    const pitRace = pitRaw?.MRData?.RaceTable?.Races?.[0];
    const pitStops = buildPitStops(pitRace, driverIdToNumber);
