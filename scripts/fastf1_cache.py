...
def read_cache_json(path: Path) -> dict[str, Any] | None:
    try:
...
def find_openf1_session(event: Any, openf1_sessions: list[dict[str, Any]]) -> dict[str, Any] | None:
    event_text = norm(f"{event.get('EventName', '')} {event.get('Location', '')} {event.get('Country', '')}")
    event_round = int(event.get("RoundNumber", 0) or 0)

    # OpenF1 does not expose round number, so match by event/location/country text.
    best = None
    best_score = 0
    for session in openf1_sessions:
        ...
        for token in [norm(event.get("EventName")), norm(event.get("Location")), norm(event.get("Country"))]:
            if token and (token in text or text in token):
                score += 1
        if event_round and abs(event_round - int(session.get("meeting_key", event_round) or event_round)) == 0:
            score += 0
        if score > best_score:
            best = session
            best_score = score
...
def build_drivers(session: Any) -> list[dict[str, Any]]:
    if session.results is None or session.results.empty:
        driver_numbers = sorted(set(int(x) for x in session.laps["DriverNumber"].dropna().unique())) if not session.laps.empty and "DriverNumber" in session.laps.columns else []
        return [
            {
    ...
    for _, row in session.results.iterrows():
        number = result_position(row.get("DriverNumber")) or result_position(row.get("DriverId")) or result_position(row.get("Position"))
        if not number:
            continue
...
def build_laps(session: Any) -> list[dict[str, Any]]:
    laps = []
...
def build_pit_stops(session: Any) -> list[dict[str, Any]]:
    pits = []
    if session.laps is None or session.laps.empty:
        return pits
    for _, lap in session.laps.iterrows():
        ...
        pits.append(
            {
                "driver_number": driver_number,
                "lap_number": result_position(lap.get("LapNumber")),
                "date": to_jsonable(lap.get("LapStartDate")),
                "lane_duration": None,
...
def build_stints(session: Any) -> list[dict[str, Any]]:
    if session.laps is None or session.laps.empty or "Stint" not in session.laps.columns:
        return []
    stints = []
    grouped = session.laps.dropna(subset=["DriverNumber", "LapNumber", "Stint"]).groupby(["DriverNumber", "Stint"], sort=True)
        ...
        stints.append(
            {
                ...
                "tyre_age_at_start": to_jsonable(group.get("TyreLife", pd.Series([None])).iloc[0]),
            }
        )
    return stints


def build_weather(session: Any) -> list[dict[str, Any]]:
    weather = []
    data = getattr(session, "weather_data", None)
        ...
        weather.append(
            {
                ...
                "air_temperature": to_jsonable(row.get("AirTemp")),
                "humidity": to_jsonable(row.get("Humidity")),
                "pressure": to_jsonable(row.get("Pressure")),
                "rainfall": to_jsonable(row.get("Rainfall")),
                "track_temperature": to_jsonable(row.get("TrackTemp")),
                "wind_direction": to_jsonable(row.get("WindDirection")),
                "wind_speed": to_jsonable(row.get("WindSpeed")),
            }
        )
    return weather
    ...
    try:
        ...
        for _, row in pos.iloc[::step].iterrows():
            points.append(
                {
                    "date": to_jsonable(row.get("Time")),
                    "driver_number": result_position(fastest.get("DriverNumber")),
        ...
        summary.update(
            {
                ...
                "best_lap": best,
                "best_lap_text": driver.get("best_lap_text") or display_timedelta(best),
                "average_lap": avg,
                "median_lap": med,
                "pit_stop_count": len(driver_pits),
                "average_pit_stop": sum(pit_durations) / len(pit_durations) if pit_durations else None,
                "fastest_pit_stop": min(pit_durations) if pit_durations else None,
                "pit_stops": driver_pits,
                "stints": driver_stints,
                "race_control_events": [],
                "positions": [],
                "position_delta": driver.get("starting_position") - driver.get("finishing_position") if driver.get("starting_position") and driver.get("finishing_position") else None,
            }
        )
...
def build_summary(session_info: dict[str, Any], drivers: list[dict[str, Any]], laps: list[dict[str, Any]], pits: list[dict[str, Any]], weather: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "total_laps_with_times": len([lap for lap in laps if lap.get("lap_duration")]),
        "dnf_count": len([d for d in drivers if d.get("dnf")]),
        "dns_count": len([d for d in drivers if d.get("dns")]),
        "dsq_count": len([d for d in drivers if d.get("dsq")]),
        "pit_stop_count": len(pits),
        "average_pit_stop": None,
        "slowest_pit_stop": None,
        "fastest_lap": fastest_driver,
...
def cache_race(year: int, event: Any, openf1_sessions: list[dict[str, Any]], telemetry: bool) -> None:
    ...
    drivers = build_drivers(ff_session)
    laps = build_laps(ff_session)
    pits = build_pit_stops(ff_session)
    stints = build_stints(ff_session)
    weather = build_weather(ff_session)
    track_map = build_track_map(ff_session) if telemetry else {"source_driver_number": None, "source_driver_name": None, "points": [], "note": "FastF1 telemetry не загружалась. Запусти с --telemetry для карты."}
    driver_summaries = enrich_driver_summaries(drivers, laps, pits, stints)

    # Re-apply best lap stats from generated summaries to top-level drivers.
    ...
    dashboard = {
        ...
        "data_quality": {
            ...
            "fastf1_cache": True,
            "note": "Данные загружены локально через FastF1. Это не мок: исторические данные сохранены в data/openf1-cache/race-dashboard.",
        },
    }
