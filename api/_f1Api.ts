...
function formatPit(pit: any) {
  return {
    ...
    lap_number: pit.lap_number ?? null,
    date: pit.date ?? null,
    lane_duration: pit.lane_duration ?? null,
    stop_duration: pit.stop_duration ?? null,
  };
}
...
function formatEvent(event: any) {
  return {
    ...
    driver_number: event.driver_number ?? null,
  };
}
...
function buildDriverSummary(driver: any, data: any) {
  ...
  const validLapTimes = driverLaps
    .filter((lap: any) => lap.lap_duration && lap.lap_duration > 0 && !lap.is_pit_out_lap)
    .map((lap: any) => Number(lap.lap_duration));
  ...
  const driverPositions = compact(
    data.positions
      .filter((position: any) => position.driver_number === driver.driver_number)
      .map((position: any) => ({ date: position.date, position: position.position })),
    120,
  );
  ...
  return {
    ...
    position_delta:
      Number.isFinite(driver.starting_position) && Number.isFinite(driver.finishing_position)
        ? Number(driver.starting_position) - Number(driver.finishing_position)
        : null,
  };
}
...
function analyzeDriverIssues(driver: any, summary: any, allPitDurations: number[]) {
  ...
  for (const event of summary.race_control_events || []) {
    if (messageLooksLikeIssue(event.message)) {
      issues.push(buildIssue(driver, {
        type: /penalty/i.test(event.message) ? "penalty" : "race_control",
        severity: /penalty|collision|unsafe/i.test(event.message) ? "high" : "medium",
        lap_number: event.lap_number ?? null,
        message: `Race Control отметил событие: ${event.message}`,
        source: "race_control",
      }));
    }
  }
  ...
  if (driver.dnf) {
    issues.push(buildIssue(driver, {
      type: "dnf",
      severity: "high",
      message: "Пилот не финишировал. Это проблемный момент гонки, но причина должна подтверждаться событиями Race Control.",
  ...
  if (driver.dns) {
    issues.push(buildIssue(driver, {
      type: "dns",
      severity: "high",
      message: "Пилот не стартовал в гонке.",
      source: "session_result",
    }));
  }
  ...
  if (driver.dsq) {
    issues.push(buildIssue(driver, {
      type: "dsq",
      severity: "high",
      message: "Пилот был дисквалифицирован.",
      source: "session_result",
    }));
  }

  if (typeof summary.position_delta === "number" && summary.position_delta < -4) {
    issues.push(buildIssue(driver, {
      type: "position_loss",
      severity: summary.position_delta < -8 ? "high" : "medium",
      message: `Пилот потерял ${Math.abs(summary.position_delta)} позиций относительно старта. Это не обязательно ошибка, но заметный провал результата.`,
      source: "calculated",
    }));
  }
  ...
  if (avgPit) {
    for (const pit of summary.pit_stops || []) {
      ...
      if (Number.isFinite(duration) && duration > avgPit * 1.25) {
        issues.push(buildIssue(driver, {
          type: "slow_pit_stop",
          severity: duration > avgPit * 1.5 ? "high" : "medium",
          lap_number: pit.lap_number ?? null,
          message: `Пит-стоп на круге ${pit.lap_number ?? "—"} был заметно медленнее среднего по гонке.`,
          source: "calculated",
        }));
      }
    }
...
function buildRaceSummary(session: any, drivers: any[], data: any) {
  const top3 = drivers
    .filter((driver) => Number.isFinite(driver.finishing_position))
    .sort((a, b) => a.finishing_position - b.finishing_position)
    .slice(0, 3);
  const lapTimes = data.laps
    .filter((lap: any) => lap.lap_duration && lap.lap_duration > 0 && !lap.is_pit_out_lap)
    .map((lap: any) => ({ driver_number: lap.driver_number, lap_number: lap.lap_number, lap_duration: lap.lap_duration }));
  ...
  const slowestPitStop = data.pitStops
    .filter((pit: any) => Number.isFinite(Number(pit.stop_duration ?? pit.lane_duration)))
    .sort((a: any, b: any) => Number(b.stop_duration ?? b.lane_duration) - Number(a.stop_duration ?? a.lane_duration))[0] || null;
  ...
  return {
    ...
    total_laps_with_times: data.laps.length,
    ...
    pit_stop_count: data.pitStops.length,
    average_pit_stop: average(pitDurations),
    slowest_pit_stop: slowestPitStop,
    fastest_lap: fastestLap,
    race_control_event_count: data.events.length,
    weather_latest: data.weather[data.weather.length - 1] || null,
  };
}
...
function normalizeLocation(points: any[]) {
  ...
  return points.map((point) => ({
    ...
    x: Number((((Number(point.x) - minX) / width) * 100).toFixed(3)),
    y: Number((((Number(point.y) - minY) / height) * 100).toFixed(3)),
    z: point.z ?? null,
  }));
}
...
export async function getOpenF1RaceDashboard(sessionKey: number) {
  ...
  const rawLocation = mapDriver
    ? await optionalFetch(`${OPENF1_BASE}/location?session_key=${sessionKey}&driver_number=${mapDriver.driver_number}`, [], 12_000)
    : [];
  const sampledLocation = compact(rawLocation, 1200);
  const trackMap = {
    source_driver_number: mapDriver?.driver_number ?? null,
    source_driver_name: mapDriver?.full_name ?? null,
    points: normalizeLocation(sampledLocation),
    note: sampledLocation.length
      ? "Карта построена по x/y координатам OpenF1 одного пилота и подходит для схемы трассы/replay."
      : "OpenF1 не вернул location-точки для карты этой сессии.",
  };
